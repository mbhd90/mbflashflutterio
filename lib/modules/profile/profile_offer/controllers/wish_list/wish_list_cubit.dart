...
class WishListCubit extends Cubit<WishListState> {
  ...
  Future<void> getWishList() async {
    if (_loginBloc.userInfo == null) {
      emit(WishListStateError(Language.pleaseLoginFirst, 1000));
      return;
    }
  ...
  Future<Either<Failure, String>> removeWishList(WishListModel item) async {
    if (_loginBloc.userInfo == null) {
      return left(ServerFailure(Language.pleaseLoginFirst, 1000));
    }
...
/*  Future<void> addWishList(int id) async {
    ...
    WishListModel? item;
    if (_loginBloc.userInfo == null) {
      // return left(const ServerFailure(Language.pleaseLoginFirst, 1000));
      return;
    }
    emit(const WishListStateLoading());
