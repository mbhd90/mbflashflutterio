...
class FilterModelDto extends Equatable {
  ...
  Map<String, dynamic> toMap(String category, String shop_name, int page, int perPage) {
    ...
    result.addAll({'page': page.toString()});
    result.addAll({'per_page': perPage.toString()});
    if(category.isNotEmpty){
      result.addAll({'category': category});
    }

    if(shop_name.isNotEmpty){
      result.addAll({'shop_name': shop_name});
    }
  ...
  String toJson() => json.encode(toMap("", "", 1, 10));
